...
> Mốc tham chiếu: Camgoz 2018 end-to-end **9.58**; Sign Language Transformers 2020 **~21.8**; SOTA gloss-free hiện tại **~22–27** BLEU-4. Đề tài **báo cáo đầy đủ ở 3 mức subset 5 / 10 / 25%** (train %split, dev/test full) với model nhỏ, nên **con số tuyệt đối không phải mục tiêu** - mục tiêu là **ΔBLEU giữa CE-only và CE+RL từ cùng checkpoint gốc** (và ΔBLEU đó thay đổi ra sao khi tăng dữ liệu - Exp 11), cộng khoảng cách tới baseline sàn.
>
> **📊 Kết quả thực nghiệm (toàn bộ sweep đã chạy) - phát hiện chính là một hiệu ứng theo quy mô dữ liệu:** Ở **5%** không có gain RL tin cậy (SCST 4.31 vs CE 4.16 = +0.15, nhỏ hơn nhiễu ~0.5; một lần chạy lại cùng cấu hình chỉ đạt 3.75). Nhưng ở **10 / 25%** gain RL **xuất hiện và tăng dần**: best-Δ (best RL − XE) = **+0.15 → +0.76 → +0.63**, do **MRT** (5.80/6.49) và **PPO** gánh, không phải SCST thuần - **đảo ngược H1** (vốn dự đoán gain lớn hơn ở data ít). Bảng xếp hạng **encoder cũng đảo**: ở 5% TCN nhất (5.40) và graph tệ nhất, nhưng ở **25% Graph-Transformer 7.64 / GCN 7.27 vượt Transformer (5.94, giờ tệ nhất)**, ST-GCN 6.74 > Transformer với ít hơn 23% tham số (**H4 sai ở 5%, đúng ở 25%**). Baseline sàn: `base_empty` **0.0**, `base_most_frequent` **0.19**. Toàn bộ hệ under-generate (len_ratio ≈ 0.10) nên BLEU bị brevity bóp, không lỗi lặp (rep ≈ 0) → penalty reward chỉ tổn hại BLEU (H2 không được ủng hộ). **Thử nghiệm phụ How2Sign** (I3D vs pose, 10/25/50%) tái lập cùng kết luận trên ngôn ngữ ASL: I3D ≫ pose, RL chỉ thêm một chút.
...
- **Experiment 1 - CE vs SCST**: cùng checkpoint gốc, báo cáo đầy đủ ở **3 mức subset 5 / 10 / 25%** (đã chạy toàn bộ). **Câu chuyện chính** - và ΔBLEU tăng theo data (Exp 11).
...
2. **`selection` / `temp`** (đối chứng cho frame-selection F.8 và decode-temperature F.5) - đã **chuyển sang hướng phát triển** (`docs/2_Huong_Phat_Trien.md`), **KHÔNG** nằm trong report hiện tại. Report chỉ có sàn `trivial`.

> [!important] Câu chốt của slide 10
> "Nguyên tắc: **mọi con số phải đọc tương đối so với sàn, không bao giờ đọc tuyệt đối**. BLEU-4 5.40 của TCN (5%) chỉ có nghĩa khi biết `base_most_frequent` chỉ đạt **0.19** và `base_empty` **0.0** - model vượt sàn rõ ràng, nhưng tuyệt đối vẫn thấp vì subset nhỏ; ở 25% các encoder tốt nhất đã lên ~7.5."
> *(Số thật từ `analysis_out/overview.md` - toàn bộ sweep 5/10/25%; xem §K.)*
...
**📊 Đối chiếu với kết quả THẬT của toàn bộ sweep 5/10/25% (chứng minh mình đo trung thực, không kết luận ngược):**
- **H1** - *một nửa bị ĐẢO*: RL **có** thắng CE, nhưng gain **TĂNG theo data**, không giảm. Best-Δ (best RL − XE) = **+0.15 (5%) → +0.76 (10%) → +0.63 (25%)**. Ở 5% Δ nằm trong nhiễu ~0.5 nên không tin cậy; ở 10/25% thì vượt hẳn nhiễu. H1 dự đoán gain lớn hơn ở data ÍT (exposure bias) - thực tế ngược lại: RL cần đủ data để ước lượng advantage ổn định (khớp Kiegeland 2021). Người gánh gain **không phải SCST thuần** (Δ chỉ +0.15/+0.19/+0.08) mà là **MRT** (5.80/6.49) và **PPO** (Δ +0.03→+0.24→+0.45).
- **H2** - *không được ủng hộ ở mọi mức*: reward **BLEU thuần luôn tốt nhất** (3.98/5.19/5.96); mọi penalty đều **giảm** BLEU, rep+len sụp mạnh ở 10/25% (3.85, 4.44). Vì `rep ≈ 0` và câu đã quá ngắn (len_ratio 0.11→0.03) → không có reward-hacking để phạt, penalty là chi phí thuần trong chế độ brevity-limited này.
- **H3** - *ĐÚNG (trung lập), có lung lay nhẹ*: PPO Δ tăng tới +0.45 ở 25% khi critic có data, nhưng **không bao giờ vượt MRT** trên 1 seed → giữ verdict trung lập (khớp Kiegeland 2021).
- **H4** - *SAI ở 5%, ĐÚNG ở 25%*: ở 5% ST-GCN 3.62 < Transformer 4.31 (graph encoder tệ nhất). Nhưng graph scale tốt hơn: **ở 25% ST-GCN 6.74 > Transformer 5.94** với ít hơn 23% tham số, và **Graph-Transformer 7.64 / GCN 7.27** vượt Transformer (giờ là **tệ nhất** ở 5.94). Bảng xếp hạng encoder **đảo hoàn toàn** theo data. TCN ổn định nhất mọi mức (5.40/6.00/7.47).
- **H5** - *chưa chạy*: frame-selection đã chuyển sang hướng phát triển (`docs/2_Huong_Phat_Trien.md`).

**Bài học phương pháp:** 3/4 giả thuyết chỉ **giải đúng khi đọc qua cả sweep** - đây là lý do mạnh nhất để tổ chức nghiên cứu quanh subset sweep thay vì một mức đơn.
...
> [!question] "Sao không chạy full dataset mà lại các subset nhỏ?"
> Ba lý do: (1) ngân sách ~30 GPU-h/tuần mà RL cần chạy nhiều lần; (2) subset **không phải hạn chế bị ép mà là một trục thí nghiệm miễn phí** - chính là Experiment 11; (3) báo cáo ở **3 mức 5 / 10 / 25%** dùng chung seed cố định (lồng nhau) nên so sánh giữa các mức được kiểm soát (§G.31) - cả 3 mức đã chạy xong, 50/100% để mở rộng khi có quota.
...
- **SECONDARY (Exp 13): How2Sign** - ưu tiên hơn CSL-Daily vì tải được ngay + khác hẳn ngôn ngữ ký hiệu (ASL vs DGS) → phép thử generalization mạnh hơn. **Thí nghiệm phụ ("train thử") ĐÃ CHẠY** ở **subset 10 / 25 / 50%**, dạng so I3D feature (video) vs MediaPipe pose feature, seed cố định, work_dir RIÊNG, báo cáo TÁCH khỏi ranking PHOENIX. Kết quả: I3D ≫ pose ở mọi mức, RL chỉ thêm chút (xem Slide 10 Exp 13). Gộp trình bày bằng `make_overview.py --root how2sign=...`.
...
| 13 | Cross-dataset transfer (How2Sign 10/25/50%, I3D vs pose) | **TB (thí nghiệm phụ) - ĐÃ CHẠY** | ✅ I3D ≫ pose; RL thêm chút; báo cáo tách khỏi PHOENIX |
...
**Exp 11 - Data size ablation (KẾT QUẢ CHÍNH)**: ΔBLEU (best RL − CE) theo subset = **+0.15 (5%) → +0.76 (10%) → +0.63 (25%)**. Hai giả thuyết cạnh tranh là RL lợi hơn ở data ít (exposure bias nặng) vs data nhiều (advantage ổn định) → **dữ liệu ủng hộ vế thứ hai**: gain xuất hiện và tăng theo data, đảo ngược H1. Ở 5% Δ nằm trong nhiễu ~0.5; ở 10/25% vượt hẳn nhiễu, do MRT/PPO gánh (advantage ước lượng giàu hơn baseline greedy đơn của SCST).
...
**Exp 13 - Cross-dataset transfer (How2Sign) — thí nghiệm phụ / "train thử" (ĐÃ CHẠY)**: cùng recipe CE→SCST trên How2Sign (ASL) ở **subset 10 / 25 / 50%**, ở dạng **so sánh 2 modality feature: I3D (video, 1024-d) vs MediaPipe pose (99-d)**. Kết quả (test BLEU-4): **I3D** XE→SCST = 2.11→2.22 / 3.28→3.52 / 4.25→4.35; **pose** = 1.67→1.59 / 1.76→1.93 / 1.62→1.25. Ba đọc số khớp PHOENIX: (1) **I3D ≫ pose** ở mọi mức và chỉ I3D scale theo data (pose phẳng ~1.6-1.9) → trần pose-only là có thật, thúc đẩy hướng RGB tương lai; (2) SCST−CE nhỏ và phụ thuộc data (I3D +0.1..+0.24; pose thất thường −0.37..+0.17); (3) kết luận "RL chỉ thêm một chút, tuỳ quy mô" **tái lập qua ngôn ngữ và modality khác** - mạnh hơn bất kỳ ô PHOENIX đơn lẻ nào. Báo cáo **tách khỏi ranking PHOENIX**, gộp trình bày bằng `make_overview.py --root how2sign=...`.
...
> dạng) và **một orchestrator** `run_all.py` chạy TOÀN BỘ ma trận cho 1 subset trong 1 lệnh. Thiết
> kế báo cáo = **3 mức PHOENIX 5 / 10 / 25%** (đã chạy đủ; dev/test full) + **thí nghiệm phụ
> How2Sign 10/25/50%** (I3D vs pose, work_dir riêng). Đọc số run thật ở `comparison_table.csv`; gộp mọi mức+dataset
> thành 1 bảng pivot bằng `scripts/make_overview.py` (→ `analysis_out/overview.md`).
