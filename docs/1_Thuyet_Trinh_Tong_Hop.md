...
> Mốc tham chiếu: Camgoz 2018 end-to-end **9.58**; Sign Language Transformers 2020 **~21.8**; SOTA gloss-free hiện tại **~22–27** BLEU-4. Đề tài **báo cáo ở 3 mức subset 5 / 10 / 25%** (train %split, dev/test full; 5% đã chạy làm mốc) với model nhỏ, nên **con số tuyệt đối không phải mục tiêu** - mục tiêu là **ΔBLEU giữa CE-only và CE+RL từ cùng checkpoint gốc** (và ΔBLEU đó thay đổi ra sao khi tăng dữ liệu - Exp 11), cộng khoảng cách tới baseline sàn.
>
...
- **Experiment 1 - CE vs SCST**: cùng checkpoint gốc, báo cáo ở **3 mức subset 5 / 10 / 25%** (5% đã chạy làm mốc). **Câu chuyện chính.**
...
- **SECONDARY (Exp 13): How2Sign** - ưu tiên hơn CSL-Daily vì tải được ngay + khác hẳn ngôn ngữ ký hiệu (ASL vs DGS) → phép thử generalization mạnh hơn. **Thí nghiệm phụ ("train thử")** ở **subset 10 / 25%**, seed cố định, train vào work_dir RIÊNG (tách dataset), báo cáo TÁCH khỏi ranking PHOENIX. Pose trích bằng `extract_poses.py --mode video` (How2Sign là video thật). Gộp vào bảng chung bằng `make_overview.py --root how2sign=...`.
...
| 13 | Cross-dataset transfer (How2Sign 10/25%) | **TB (thí nghiệm phụ)** | ⚠️ pose `--mode video` sẵn; train work_dir riêng, gộp bằng `make_overview.py` |
...
**Exp 13 - Cross-dataset transfer (How2Sign) — thí nghiệm phụ / "train thử"**: chạy cùng pipeline pose→text + CE→RL trên How2Sign (ASL) ở **subset 10 / 25%**, so train-from-scratch cùng subset. Mục tiêu: xem kết luận PHOENIX có sống sót qua một ngôn ngữ ký hiệu khác không (ASL vs DGS). Train vào **work_dir riêng** để tách dataset; báo cáo **tách khỏi ranking PHOENIX** (không trộn chung), rồi gộp trình bày bằng `make_overview.py --root how2sign=...`. Pose trích `--mode video` (How2Sign có file video thật). BLEU baseline thấp (~8-12) → reward thưa, coi là probe chứ không phải claim chính; kết quả null vẫn có ý nghĩa.
...
> kế báo cáo = **3 mức PHOENIX 5 / 10 / 25%** (5% đã chạy làm mốc; dev/test full) + **thí nghiệm phụ
> How2Sign 10/25%** (work_dir riêng). Đọc số run thật ở `comparison_table.csv`; gộp mọi mức+dataset
> thành 1 bảng pivot bằng `scripts/make_overview.py` (→ `overview.md`).
