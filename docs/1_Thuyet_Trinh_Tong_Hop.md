...
> Quy trình hiện tại dùng **2 notebook Kaggle riêng** (`KAGGLE_NOTEBOOK_EXTRACT.ipynb` rồi
> `KAGGLE_NOTEBOOK.ipynb`) và **một orchestrator** `run_all.py` chạy TOÀN BỘ ma trận thí nghiệm cho
> 1 subset trong 1 lệnh (không còn chạy cell-by-cell như bản 37-cell cũ). Subset chuẩn giờ là
> **25/50/100%** — mức subset chuẩn dùng xuyên suốt (narrative RQ/slide đã đồng bộ theo mức này).
> Đọc số run thật ở `comparison_table.csv`, không suy từ % cũ.

### K.0. Tóm tắt

| Ưu tiên | Chạy gì | Ra số gì | Ước tính |
|---|---|---|---|
| **🔴 BƯỚC 0** | `KAGGLE_NOTEBOOK_EXTRACT.ipynb` (kernel CPU-only riêng) | `.npz` cho toàn corpus | **10–20h CPU** (không tốn quota GPU) |
| **🔴 BẮT BUỘC** | `run_all.py --subset 0.25` | Baseline sàn + Exp 1/4/7/8/9/11/15 + P7, TẤT CẢ ở 25% | ~25-30h GPU (vừa 1 tuần quota) |
| 🟡 Khi có thêm quota | `run_all.py --subset 0.5` | Cùng ma trận, subset 50% — Exp 11 (data-size ablation) | lớn hơn 25% đáng kể |
| 🟡 Khi có thêm quota | `run_all.py --subset 1.0` | Cùng ma trận, subset 100% | rất lớn — trải nhiều session/tuần, dùng `--groups` để giới hạn nếu cần |

**Nếu chỉ có 1 tuần quota:** BƯỚC 0 rồi `run_all.py --subset 0.25` (mặc định `--groups all`) là đủ
để có toàn bộ bảng so sánh chính (Exp 1/4/7/8/9/11/15 + P7) ở 1 mức subset — đủ bảo vệ luận điểm
chính. 50%/100% là mở rộng thêm khi có quota, không bắt buộc phải chạy ngay.

### K.1. BƯỚC 0 — Extract pose (`KAGGLE_NOTEBOOK_EXTRACT.ipynb`)
...
vượt quá 1 session — xem trực tiếp `KAGGLE_NOTEBOOK_EXTRACT.ipynb`, không lặp lại lệnh ở đây.
...
### K.2. BẮT BUỘC — TOÀN BỘ ma trận cho 1 subset (`KAGGLE_NOTEBOOK.ipynb`)
...
7. P7 two-stage (pose→gloss CTC→NMT).
8. Selection-policy (F.6/F.8/F.9) + baseline counter (Exp 8).
9. Decode-policy (F.5) + baseline fixed-temp.
10. Latency (Exp 15) cho 6 encoder.
11. `aggregate_results` → `comparison_table.csv/.md` (tự chạy lại ở CUỐI MỖI lần gọi `run_all.py`,
    kể cả khi 1 vài bước ở trên lỗi/bị skip — luôn phản ánh đúng trạng thái hiện tại).
12. `make_report` → `report/tables/*.csv/.md/.tex` (6 bảng đã lọc theo câu hỏi so sánh + 3 bảng
    ...
    sánh thuật toán) — cũng tự chạy lại mỗi lần, dùng luôn để viết báo cáo/paper, không cần script
    riêng. Chạy tay: `python scripts/make_report.py --work_dir /kaggle/working`.

Muốn giới hạn phạm vi (debug nhanh, hoặc cố tình bỏ bớt để tiết kiệm quota): `--groups
core,encoders,algos,ablations,reward,twostage,selection,decode,latency` (mặc định `all`).
...
```bash
python main.py --subset 0.25 --encoder transformer --algo scst --phase all --tag run1
python main.py --subset 0.25 --encoder transformer --algo scst --phase rl --tag rw_bleu_only \
    --xe_ckpt /kaggle/working/run1_transformer_subset25/best_xe.pt   # tái dùng XE tag khác
python main_twostage.py --subset 0.25 --encoder transformer
python scripts/eval_baselines.py --kind trivial --subset 0.25
python scripts/measure_latency.py --ckpt /kaggle/working/run1_transformer_subset25/best_xe.pt --encoder transformer
```
