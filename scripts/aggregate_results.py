...
def _test_bleu(entry: dict):
    """Lấy test_bleu4. KHÔNG dùng `a or b` -- BLEU 0.0 là giá trị hợp lệ (vd base_empty của
    scripts/eval_baselines.py) sẽ bị `or` nuốt thành None."""
    return entry.get("test_bleu4")
...
def collect(work_dir: str):
    ...
    for run_dir in sorted(glob.glob(os.path.join(work_dir, "*"))):
        ...
        # test_results.json có key chưa khớp history nào (vd baseline sàn base_empty/
        # base_most_frequent chỉ có test_bleu4, không có 1 vòng epoch riêng để tạo history).
